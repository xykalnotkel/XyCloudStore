...
export async function bersihkanAkun(env, user) {
  ...
  await env.DB.batch([
    ...
    q("UPDATE topup_credit SET user_id='dihapus' WHERE user_id=?", id),
    q("UPDATE topup SET user_id='dihapus',bukti=NULL,checkout_url=NULL,payment_qr=NULL,payment_code=NULL WHERE user_id=?", id),
