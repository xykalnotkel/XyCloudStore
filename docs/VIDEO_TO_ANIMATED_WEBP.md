...
## 7. Cara Test MP4 → WebP (Lokal & Cloudinary)

### 7.1 Test Lokal dengan `tools/konversi_webp_animasi.sh`

```bash
# 1. Siapkan video sample 5-10 detik (MP4)
ls -lh sample.mp4

# 2. Jalankan script konversi Discord-style
chmod +x tools/konversi_webp_animasi.sh
./tools/konversi_webp_animasi.sh sample.mp4 output_discord.webp

# 3. Bandingkan dengan GIF (opsional)
./tools/konversi_webp_animasi.sh sample.mp4 output_discord.webp --compare-gif

# Output contoh:
# Input : sample.mp4
# Output: output_discord.webp
# Spec  : 480px, 20 FPS, 5 detik, q75, loop infinite, alpha 8-bit
# Durasi sumber: 8.2s, mulai dari: 1.6s (5 detik)
# ...
# --- Hasil ---
# -rw-r--r-- 1 user user 890K output_discord.webp

# 4. Cek animasi
xdg-open output_discord.webp
# atau
ffprobe -v error -show_entries stream=codec_name,width,height,r_frame_rate,duration -of default=nw=1 output_discord.webp
```

**Test batch semua MP4 di folder:**

```bash
for f in *.mp4; do ./tools/konversi_webp_animasi.sh "$f" "${f%.mp4}.webp"; done
ls -lh *.webp
```

### 7.2 Test Cloudinary Transform (API)

**Upload manual via curl:**

```bash
# Dapatkan signature dari dashboard Cloudinary atau pakai API key langsung (test only)
curl -X POST https://api.cloudinary.com/v1_1/<cloud_name>/video/upload \
  -F file=@sample.mp4 \
  -F folder=xycloudstore/banner-profil \
  -F eager="f_webp,fl_awebp,fl_animated,w_480,c_limit,fps_20,du_5,q_auto:good,e_loop|f_gif,w_480,c_limit,fps_15,du_5,q_auto,fl_lossy,fl_animated,e_loop" \
  -F api_key=<key> \
  -F timestamp=$(date +%s) \
  -F signature=<sig> \
  -F api_secret=<secret>
...
# {
#   "secure_url": "https://res.cloudinary.com/.../video/upload/v123/xycloudstore/banner-profil/abc.mp4",
#   "eager": [
#     {"secure_url": "https://.../f_webp,fl_awebp,fl_animated,.../abc.webp", "format": "webp", "bytes": 890123},
#     {"secure_url": "https://.../f_gif,...,fl_lossy,fl_animated/.../abc.gif", "format": "gif", "bytes": 2100000}
#   ]
# }
```

**Test via API XyCloudStore (otomatis WebP + GIF):**

```bash
# 1. Login dapat token
TOKEN=$(curl -s https://api.xycloud.my.id/api/auth/login -H "Content-Type: application/json" -d '{"email":"test@test.com","password":"123456"}' | jq -r .token)

# 2. Convert MP4 base64 ke data URI
DATA_URI=$(python3 -c "import base64,sys; data=open('sample.mp4','rb').read(); print('data:video/mp4;base64,'+base64.b64encode(data).decode())")

# 3. Upload sebagai banner media
curl -X POST https://api.xycloud.my.id/api/me/banner-media \
...
#   "banner_media": {
#     "tipe": "video",
#     "url": "https://res.cloudinary.com/.../f_webp,fl_awebp,fl_animated,.../abc.webp",
#     "webp": "https://res.cloudinary.com/.../f_webp,fl_awebp,fl_animated,.../abc.webp",
#     "gif": "https://res.cloudinary.com/.../f_gif,.../abc.gif"
#   }
# }
```

**Verifikasi di Flutter:**

```dart
// BannerMedia.displayUrl akan otomatis pakai webp > gif > url
final media = BannerMedia.fromJson(json);
print(media.webp); // primary Discord-style
print(media.gif);  // fallback
CachedNetworkImage(
  imageUrl: media.displayUrl, // webp
  imageBuilder: (c, p) => Image(image: p, gaplessPlayback: true),
)
```
...
```bash
curl -sI https://res.cloudinary.com/.../f_webp,fl_awebp,fl_animated,.../abc.webp | grep -i content-length
curl -sI https://res.cloudinary.com/.../f_gif,.../abc.gif | grep -i content-length
# WebP biasanya 60-70% lebih kecil
```
...
| Masalah | Solusi |
|---------|--------|
| `ffmpeg: command not found` | `sudo apt install ffmpeg` atau `brew install ffmpeg` |
| WebP tidak animasi | Pastikan `-loop 1` dan `-vcodec libwebp`, bukan `webp` biasa |
| File > 5 MB | Potong durasi `-t 5`, turunkan FPS `fps=15`, quality `q60` |
| Cloudinary eager kosong | Cek `api_secret` & `timestamp`, pastikan `resource_type=video` |
| Flutter tidak loop | Tambah `gaplessPlayback: true` di `Image` |
| Alpha jelek di GIF | Pakai WebP (8-bit alpha), jangan GIF (1-bit) |
...
*Last updated: 2026-09-19 Batch Q — Discord-style Animated WebP + Testing Guide*
